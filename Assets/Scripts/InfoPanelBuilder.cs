...
public class InfoPanelBuilder : MonoBehaviour
{
    ...
    [HideInInspector] public TextMeshProUGUI txtStressMeta;
    [HideInInspector] public TextMeshProUGUI txtMoodMeta;
    ...
    [HideInInspector] public TextMeshProUGUI txtAffinityMeta;
    ...
    private static readonly Color HeaderColor = new Color(0.38f, 0.24f, 0.14f, 0.98f);
    private static readonly Color CardBgColor = new Color(0.98f, 0.94f, 0.86f, 0.99f);
    private static readonly Color SectionBgColor = new Color(0.97f, 0.91f, 0.82f, 0.98f);

    private static readonly Color ButtonNormal = new Color(0.62f, 0.43f, 0.23f, 1.0f);
    private static readonly Color TabActiveColor = new Color(0.71f, 0.50f, 0.24f, 1.0f);
    private static readonly Color TabInactiveColor = new Color(0.82f, 0.71f, 0.55f, 1.0f);

    private static readonly Color TextWhite = new Color(0.20f, 0.14f, 0.10f);
    private static readonly Color TextGold = new Color(1.0f, 0.88f, 0.46f);
    private static readonly Color TextGray = new Color(0.44f, 0.31f, 0.22f);

    private static readonly Color OverlayColor = new Color(0f, 0f, 0f, 0.40f);
    private static readonly Color SeparatorColor = new Color(0.72f, 0.58f, 0.42f, 1.0f);
    ...
    private void CreateMainPanel()
    {
        ...
        rt.sizeDelta = new Vector2(1180, 760);
    ...
    private void CreateHeader()
    {
        ...
        txtTitle = CreateTMPText("Title", header.transform, "个人信息", 24f, TextGold,
            TextAlignmentOptions.Center, new Vector2(720, 64));
        ...
        // 关闭按钮
        btnClose = CreateButton("BtnClose", header.transform, "X", ButtonNormal, new Vector2(44, 44));
    ...
    private void CreateTabBar()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            tabButtons[i] = CreateButton($"Tab{i}", tabBar.transform, tabNames[i], TabInactiveColor, new Vector2(360, 52));
        }
    }
    ...
    private void CreatePlayerInfoPanel()
    {
        ...
        rt.offsetMax = new Vector2(-18, -124); // 减去Header和TabBar高度并保留边距

        // 创建ScrollView
        ScrollRect scrollRect = CreateScrollView("ScrollView", playerInfoPanel.transform, new Vector2(1144, 618));
    ...
    private void CreatePlayerBasicInfoSection(Transform parent)
    {
        ...
        txtPlayerName = CreateTMPText("PlayerName", section.transform, "姓名：XXX", 18f, TextWhite,
            TextAlignmentOptions.Left, new Vector2(1020, 30));
    ...
    private void CreatePlayerStatusSection(Transform parent)
    {
        ...
        stressRT.sizeDelta = new Vector2(1020, 30);
        ...
        GameObject stressBarBg = CreatePanel("StressBarBg", stressRow.transform, new Color(0.71f, 0.80f, 0.86f, 0.9f));
        RectTransform stressBarBgRT = stressBarBg.GetComponent<RectTransform>();
        stressBarBgRT.sizeDelta = new Vector2(660, 20);
        ...
        fillRT.sizeDelta = new Vector2(660, 20);
        imgStressBar = stressBarFill.AddComponent<Image>();
        imgStressBar.color = new Color(0.9f, 0.3f, 0.3f);
        imgStressBar.type = Image.Type.Filled;
        imgStressBar.fillMethod = Image.FillMethod.Horizontal;
        imgStressBar.fillOrigin = 0;
        imgStressBar.fillAmount = 0f;

        txtStress = CreateTMPText("StressValue", stressRow.transform, "40%", 16f, TextWhite,
            TextAlignmentOptions.Left, new Vector2(100, 30));

        txtStressMeta = CreateTMPText("StressMeta", section.transform, "状态稳定", 14f, TextGray,
            TextAlignmentOptions.Left, new Vector2(1020, 24));
        ...
        moodRT.sizeDelta = new Vector2(1020, 30);
        ...
        GameObject moodBarBg = CreatePanel("MoodBarBg", moodRow.transform, new Color(0.71f, 0.80f, 0.86f, 0.9f));
        RectTransform moodBarBgRT = moodBarBg.GetComponent<RectTransform>();
        moodBarBgRT.sizeDelta = new Vector2(660, 20);
        ...
        moodFillRT.sizeDelta = new Vector2(660, 20);
        imgMoodBar = moodBarFill.AddComponent<Image>();
        imgMoodBar.color = new Color(0.3f, 0.8f, 0.5f);
        imgMoodBar.type = Image.Type.Filled;
        imgMoodBar.fillMethod = Image.FillMethod.Horizontal;
        imgMoodBar.fillOrigin = 0;
        imgMoodBar.fillAmount = 0f;

        txtMood = CreateTMPText("MoodValue", moodRow.transform, "70%", 16f, TextWhite,
            TextAlignmentOptions.Left, new Vector2(100, 30));

        txtMoodMeta = CreateTMPText("MoodMeta", section.transform, "再提升 10 进入高涨", 14f, TextGray,
            TextAlignmentOptions.Left, new Vector2(1020, 24));
    ...
    private void CreatePlayerHiddenAttrsSection(Transform parent)
    {
        ...
        txtHiddenAttrs = CreateTMPText("HiddenAttrs", section.transform, "黑暗值：5    负罪感：10    幸运：50", 16f, TextGray,
            TextAlignmentOptions.Left, new Vector2(1020, 50));
    ...
    private void CreateRelationshipPanel()
    {
        ...
        leftRT.sizeDelta = new Vector2(280, 0);
        ...
        rightRT.offsetMin = new Vector2(292, 0);
        rightRT.offsetMax = new Vector2(0, 0);

        // 左侧NPC列表
        ScrollRect leftScroll = CreateScrollView("NPCListScroll", leftPanel.transform, new Vector2(280, 618));
        npcListContent = leftScroll.content;

        // 右侧详情区
        ScrollRect rightScroll = CreateScrollView("NPCDetailScroll", rightPanel.transform, new Vector2(832, 618));
    ...
    private void CreateNPCDetailUI(Transform parent)
    {
        ...
        GameObject affinityBarBg = CreatePanel("AffinityBarBg", affinityRow.transform, new Color(0.71f, 0.80f, 0.86f, 0.9f));
        ...
        imgAffinityBar.type = Image.Type.Filled;
        imgAffinityBar.fillMethod = Image.FillMethod.Horizontal;
        imgAffinityBar.fillOrigin = 0;
        imgAffinityBar.fillAmount = 0f;

        txtNPCAffinity = CreateTMPText("AffinityValue", affinityRow.transform, "75 / 100", 16f, TextWhite,
            TextAlignmentOptions.Left, new Vector2(100, 30));

        txtAffinityMeta = CreateTMPText("AffinityMeta", parent, "还差 5 进入下一关系", 14f, TextGray,
            TextAlignmentOptions.Left, new Vector2(850, 22));
    ...
    private Button CreateButton(string name, Transform parent, string label, Color normalColor, Vector2 size)
    {
        ...
        TextMeshProUGUI btnText = CreateTMPText(name + "Label", btnObj.transform, label,
            18f, new Color(0.98f, 0.96f, 0.92f), TextAlignmentOptions.Center, size);
    ...
    private GameObject CreateSection(string name, Transform parent, string title)
    {
        ...
        sectionRT.sizeDelta = new Vector2(1020, 0);
        ...
        // 区块标题
        CreateTMPText("SectionTitle", section.transform, title, 18f, new Color(0.49f, 0.31f, 0.14f),
            TextAlignmentOptions.Left, new Vector2(1020, 30));
    ...
    private void CreateSeparator(Transform parent)
    {
        ...
        rt.sizeDelta = new Vector2(1020, 1);
