...
namespace MCPForUnity.Editor.Windows
{
    public class MCPForUnityEditorWindow : EditorWindow
    {
        ...
        private static void BuildDependenciesSection(VisualElement container)
        {
            ...
            var desc = new Label("部分工具组需要可选包。安装它们以解锁更多功能。");
            ...
            {
                if (!EditorUtility.DisplayDialog("安装所有依赖",
                    "这将安装 Roslyn DLLs、ProBuilder、Cinemachine 和 VFX Graph。是否继续？",
                    "全部安装", "取消")) return;
                installAllButton.SetEnabled(false);
                installAllButton.text = "安装中...";
            ...
            {
                if (!EditorUtility.DisplayDialog("卸载所有依赖",
                    "这将移除 Roslyn DLLs、ProBuilder、Cinemachine 和 VFX Graph。是否继续？",
                    "全部卸载", "取消")) return;
                uninstallAllButton.SetEnabled(false);
                uninstallAllButton.text = "移除中...";
                UninstallRoslyn();
            ...
            AddDependencyRow(content,
                "Cinemachine",
                "增强 manage_camera 的虚拟摄像机支持 (core 组)。",
                hasCinemachine,
                "已安装",
                "未安装 \u2014 摄像机工具仍可正常使用",
            ...
            AddDependencyRow(content,
                "VFX Graph",
                "在 manage_vfx 工具中启用 VisualEffect 支持 (vfx 组)。",
                hasVfxGraph,
                "已安装",
                "未安装 \u2014 VFX 工具将回退到 ParticleSystem/LineRenderer",
